    ...
    if (psx != nullptr && psx->isConnected())
    {
        ...
        if (moduleController.get(AvailableModules::motor) != nullptr)
		{
			...
			if (abs(psx->state.analog.stick.ly) > offset_val)
			{
				...
				motor->data[Stepper::Z]->speed =  sgn(stick_ly)*(int)pow(stick_ly * global_speed,2) ;
			...
			if ((abs(psx->state.analog.stick.rx) > offset_val))
			{
				...
				motor->data[Stepper::X]->speed = sgn(stick_rx)*(int)pow(stick_rx * global_speed,2) ;
			...
			if ((abs(psx->state.analog.stick.ry) > offset_val))
			{
				...
				motor->data[Stepper::Y]->speed = sgn(stick_ry)*(int)pow(stick_ry * global_speed,2) ;

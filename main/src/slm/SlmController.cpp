...
DynamicJsonDocument SlmController::act(DynamicJsonDocument ob)
{
	...
	if (DEBUG)
		Serial.println("slm_act_fct");
	isBusy = true;

	const char *slmMode = ob["slmMode"]; // "ring", "clear"
	...
	if (strcmp(slmMode, "circle"))
	{
		...
		if (ob.containsKey("posX"))
			posX = ob["posX"];
		if (ob.containsKey("posY"))
			posY = ob["posY"];
		if (ob.containsKey("radius"))
			radius = ob["radius"];
		if (ob.containsKey("color"))
			color = ob["color"];
	...
	if (strcmp(slmMode, "rect"))
	{
		...
		if (ob.containsKey("posY"))
			posY = ob["posY"];
		if (ob.containsKey("nX"))
			nX = ob["nX"];
		if (ob.containsKey("nY"))
			nY = ob["nY"];
		if (ob.containsKey("color"))
			color = ob["color"];
	...
	else if (strcmp(slmMode, "image"))
	{
		...
		int startX = ob["startX"];
		int startY = ob["startY"];
		int endX = ob["endX"];
		int endY = ob["endY"];
		...
		for (int ix = 0; ix < nX; ix++)
		{
			...
			{
				uint16_t color = ob["color"][ix * (nX) + iy]; // Implicit cast
...
DynamicJsonDocument SlmController::set(DynamicJsonDocument doc)
{
	/*
	  ...
	  //if (DEBUG) Serial.print("LED_ARRAY_PIN "); Serial.println(ob["LED_ARRAY_PIN"]);
	  LED_ARRAY_PIN = ob["LED_ARRAY_PIN"];
	  }

	  if (ob["LED_N_X"] != 0) {
	  //if (DEBUG) Serial.print("LED_N_X "); Serial.println(ob["LED_N_X"]);
	  LED_N_X = ob["LED_N_X"];
	  }

	  if (ob["LED_N_Y"] != 0) {
	  //if (DEBUG) Serial.print("LED_N_Y "); Serial.println(ob["LED_N_Y"]);
	  LED_N_Y = ob["LED_N_Y"];
	  }
	*/
}
...
{
	if (DEBUG)
		Serial.println("Initializing SLM");
	tft.initR(INITR_BLACKTAB); // Init ST7735S chip, black tab
	tft.fillScreen(ST77XX_WHITE);
	delay(500);
	tft.fillScreen(ST77XX_BLACK);
	if (DEBUG)
		Serial.println("Done Initializing SLM");
}

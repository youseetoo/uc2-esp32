...
void EspWifiController::initSoftAp()
{
    ...
    wifi_config.ap.ssid_len = strlen(wconfig->mSsid.c_str());
    wifi_config.ap.channel = 1;
    wconfig->pw.getBytes(wifi_config.ap.password, 64);
    wifi_config.ap.max_connection = 4;
    wifi_config.ap.authmode = WIFI_AUTH_WPA2_PSK;
    if (wconfig->pw.length() == 0)
    {
    ...
    log_i("wifi_init_softap finished. SSID:%s password:%s channel:%d",
             wconfig->mSsid.c_str(), wconfig->pw.c_str());
}
...
void EspWifiController::wifi_init_sta()
{
    ...
    wifi_config_t wifi_config;
    wconfig->mSsid.getBytes(wifi_config.sta.ssid,32);
    wconfig->pw.getBytes(wifi_config.sta.password, 64);
...
DynamicJsonDocument EspWifiController::wifi_scan()
{
    ESP_ERROR_CHECK(esp_netif_init());
    ESP_ERROR_CHECK(esp_event_loop_create_default());

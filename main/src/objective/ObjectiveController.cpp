		...
		*/
		...
		if (cJSON_HasObjectItem(doc, "move"))
		{
			...
			if (obj == 2)
			{
				...
				HomeMotor::startHome(sObjective, 10000, 20000, 20000, -1, 0, qid, false, 0);
				data.currentState = 2;
			}
			else if (obj == 0)
			{
		...
		if (cJSON_HasObjectItem(doc, "calibrate"))
		{
			...
			HomeMotor::startHome(sObjective, homeTimeout, speed, homeMaxspeed, homeDirection, homeEndStopPolarity, qid, false, 0);
			data.currentState = 2;
			data.isHomed = true;
		}
